...
_VALID_STATUSES = {"ok", "corrige", "inconnu"}
...
def resolve(order: OrderExtraction) -> Resolution:
    ...
    for i, p in enumerate(order.products):
        ...
        if not matched or not resolved_sku or not store.has_sku(resolved_code, resolved_sku):
            ...
            # Ne garder "corrige"/"ok" que tels quels ; tout le reste devient "ok".
            p.sku_status = "corrige" if status == "corrige" else "ok"
    ...
    if matched and n_corr:
        status = f"OK ({n_corr} SKU corrigé{'s' if n_corr > 1 else ''})"

    logger.info(
        "Résolution : client_code=%s (matched=%s), %d corrigé(s), %d inconnu(s).",
        resolved_code, matched, n_corr, n_unknown,
    )
    return Resolution(
